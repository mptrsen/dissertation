...
within a few hundred generations \citep{Dolgin2006,
Struchiner2009, Kofler2015} -- for example, the melanism in the British
peppered moth, which evolved camouflage matching the birch trees
...
have beneficial effects on the host genome, and should not be entirely
subdued.

To keep the TE population in check, host organisms have developed
defenses that remove or silence TEs. In plants, for example, a
...
stage. An epigenetic modification called DNA methylation prevents TEs
from being transcribed and thus from transposing \citep{Slotkin2007,
Lisch2009}. After transcription, proteins from the RNA interference
(RNAi) pathway can degrade messenger RNA and thereby silence TEs
\citep{Buchon2006}. Similarly, a class of non-coding RNA, so-called
Piwi-interacting RNA (piRNA) protect the integrity of the genome, in
particular in germline cells, by forming a complex with Piwi proteins,
which can bind and cleave RNA \citep{Zeng2011}. This complex can
recognize and silence target TEs in the RNA stage \citep{Siomi2011}.
Permanently inactive TEs become genetic ``cruft'' and are degraded by
genetic drift (random mutations over time) like other parts of the
genome that are not subject to selection. As a result of these extensive
